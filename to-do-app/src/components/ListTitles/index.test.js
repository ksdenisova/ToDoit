...
test("changes active list on click", () => {
  ...
  const changeActiveListMock = jest.fn();
  const wrapper = shallow(<ListTitles isLoaded={true} toDoLists={testLists} changeActiveList={changeActiveListMock}/>);

  wrapper.find('li').first().simulate('click');
 
  expect(changeActiveListMock).toHaveBeenCalledTimes(1);
})
...
test("uses 'inactive-list' style selector if list is not active", () => {
  ...
  const wrapper = shallow(<ListTitles isLoaded={true} toDoLists={testList} changeActiveList={ () => null }/>);
...
test("uses 'active-list' style selector if list is active", () => {
  ...
  const wrapper = shallow(<ListTitles isLoaded={true} toDoLists={testList} changeActiveList={ () => null } activeList={testList[0]}/>);
...
test("uses different style selectors for active and inactive lists", () => {
  ...
  const wrapper = shallow(<ListTitles isLoaded={true} toDoLists={testLists} changeActiveList={ () => null } activeList={testLists[0]}/>);
